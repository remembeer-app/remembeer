...
class BadgeService {
  ...
  ///
  /// [drink] is the catalog snapshot of the drink that was just logged.
  ...
  UserModel evaluateBadges(
    ...
    updatedUser = _checkOnetimeBadges(updatedUser, consumedAt, drink);
  ...
  UserModel _checkOnetimeBadges(
    ...
    for (final badgeId in OnetimeBadgeId.values) {
      final unlocked = switch (badgeId) {
        ...
        OnetimeBadgeId.mastiToJakDrak => _checkMastiToJakDrak(drink),
      };
  ...
  bool _checkMastiToJakDrak(DrinkSnapshot? drink) {
    if (drink == null) return false;
    return drink.name.trim().toLowerCase() ==
        mastiToJakDrakDrinkName.toLowerCase();
  }
