...
/**
 * Seeds the global drink types from assets/seed_data/drink_types.json.
 *
 * Global drink types carry userId "global", which firestore.rules forbids every
...
const COLLECTION = 'drink_types';
const GLOBAL_USER_ID = 'global';
// Firestore caps a batch at 500 operations.
const BATCH_LIMIT = 500;

const repoRoot = resolve(dirname(fileURLToPath(import.meta.url)), '..');
const seedFile = resolve(repoRoot, 'assets/seed_data/drink_types.json');
...
async function readSeedFile() {
  const drinkTypes = JSON.parse(await readFile(seedFile, 'utf8'));

  const ids = new Set();
  for (const drinkType of drinkTypes) {
    if (drinkType.userId !== GLOBAL_USER_ID) {
      throw new Error(`${drinkType.id} is not a global drink type.`);
    }
    if (ids.has(drinkType.id)) {
      throw new Error(`Duplicate id ${drinkType.id} in the seed file.`);
    }
    ids.add(drinkType.id);
  }
...
async function main() {
  ...
  const drinkTypes = await readSeedFile();
  const seededIds = new Set(drinkTypes.map((drinkType) => drinkType.id));

  const existing = await collection
    .where('userId', '==', GLOBAL_USER_ID)
    .get();

  // Drink types dropped from the seed file are soft deleted so they stop
  ...
  const created = drinkTypes.filter(
    (drinkType) => !existingById.has(drinkType.id),
  );

  console.log(
    `${drinkTypes.length} drink types in the seed file: ` +
      `${created.length} new, ${drinkTypes.length - created.length} updated, ` +
      `${retired.length} retired.`,
  );

  if (dryRun) {
    for (const drinkType of created) {
  ...
  const operations = [
    ...drinkTypes.map((drinkType) => (batch) => {
      // Reseeding must not restamp createdAt on drink types that already exist.
      const createdAt =
        existingById.get(drinkType.id)?.get('createdAt') ??
        FieldValue.serverTimestamp();

      batch.set(collection.doc(drinkType.id), {
        ...drinkType,
        createdAt,
